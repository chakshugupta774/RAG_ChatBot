...
print(" Using RAGAgent from:", RAGAgent.__module__)
...
n_best = st.sidebar.number_input("Number of answers", min_value=1, max_value=10, value=3, step=1)
...
if query:
    ...
    # st.session_state.chat_history.append({"user": query, "bot": responses})
    ...
    for i, r in enumerate(responses, 1):
        ...
        st.write(r["answer"])
        st.caption(f"Source: {r['source']} | {r['reason']}")
        if r['distance'] is not None:
            st.caption(f"Score (distance): {r['distance']:.4f}")
        st.markdown("---")
...
#         for i, ans in enumerate(chat['bot'], 1):
#             st.markdown(f"*Bot {i}:* {ans['answer'][:300]}...")